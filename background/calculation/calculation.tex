...
Information on $B_{ryt}$ and $\Delta B_{ryt}$ are provided by the field team, and encoded as multipole strengths and uncertainties in distinct time layers.  We can use an analytical function to map the multipoles onto the {r,y} space in each time layer. We can map the uncertainties in the multipoles in the same way. We can then map the two space-continuous variables onto a grid giving$B_{ryt}$ and  $\Delta B_{ryt}$. 

The formula to convert the multipole strengths, $b_i$ to $B(r,\theta)$ is

  double r = sqrt(x*x+y*y);
double rf = r/4.5;//4.5 cm aperture

\begin{equation}
B(r,y) =  n_1 +  \sum_{n=1} R^{n} (b_{2n} \cos(\theta n )  + b_{2n+1} \sin(\theta n )   )
\end{equation}
with 
\begin{equation}
R = \frac{\sqrt{x^2 + y^2}}{4.5}
\end{equation}
