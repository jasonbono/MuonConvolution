...
Letting $\Sigma_{nt} c_{nt} = \Sigma_{n} C_n $ gives
\begin{equation}
\boxed{
\left\langle B \right\rangle =
...
Assuming that the muon and field structures ($m_{nt}$ and $b_{nt}$) are uncorrelated. The variance in the nth moment at time $t$, $\sigma^2_{nt} \equiv (\Delta \left\langle B \right\rangle_{nt})^2$, is 
...
\begin{itemize}
	...
	\item Does the accumulation of error over time, as it's formulated make sense?
		\begin{itemize}
		\item Does our error calculation have built-in invariance to sample-rate?
	\end{itemize}
